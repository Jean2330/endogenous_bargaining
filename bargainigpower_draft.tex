...
\end{align*}
The terms in $\beta$ cancel exactly, so note that $M$ depends only on the model's primitives, and is a constant with respect to $\beta$. Since $\underline{y} < 0$ by construction (Section \ref{subsec:calibration}), $-\underline{y}(\mu+\kappa) > 0$, so $M > 0$ whenever $\mu \geq \kappa$, and more generally whenever $|\underline{y}|(\mu+\kappa) > \mu(\kappa-\mu)$.\\
