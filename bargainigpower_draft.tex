...
To confirm this is a maximum and not merely a critical point, differentiate $S'(\beta_t)$ once more,
\[
S''(\beta_t) = \frac{d}{d\beta_t}\left[\frac{\theta^2}{k} - \beta_t\frac{\theta^2}{k} - \gamma\beta_t\sigma^2\right] = -\frac{\theta^2}{k} - \gamma\sigma^2.
\]

Since $\theta^2/k > 0$ and $\gamma\sigma^2 \geq 0$, we have $S''(\beta_t) < 0$ for all $\beta_t$. The surplus function is strictly concave everywhere, so $\beta_t^* = \beta^{\text{eff}}$ is the unique global maximizer, confirming the problem is well-posed.
