...
\textcolor{olive}{PENDIENTE (nota general sobre este apéndice): esta es la versión estática (para un $W_t$ dado) de la Lema C.1 de la tesina, no una copia. La tesina la prueba para funciones de continuación dinámicas $V_P^b, V_A^b$ y evalúa $F$ literalmente en $\beta=0,1$; aquí encontramos que esa evaluación literal no funciona, porque en $\beta=0,1$ ya falla la restricción de participación de una de las partes (verificado con \texttt{model.py}: $\text{CE}_s<0$ cerca de $\beta=0$, $\Pi_s<0$ cerca de $\beta=1$, en la calibración base). La prueba de abajo usa en cambio el bracket factible interior, que es lo que tu propio código (\texttt{feasible\_beta\_bracket}) ya hace en la práctica. Falta: el Apéndice B de la tesina (dinámica de $\lambda(W_t)$) sigue pendiente de traer aquí, y tiene además el error de signo documentado en NOTES.md, no solo el problema de continuación vs.\ estático. El caso $\mu<\kappa$ ya se agregó como segundo caso en la prueba de abajo, con su propio argumento de cruce porque ahí $\text{CE}_t$ es cóncava (no monótona); y las condiciones de regularidad ya están formalizadas como Asunción \ref{ass:beta_bounds_reg}, con (i)-(iii) reescritas en cierre analítico como una cota en $\underline w_t$ (solo la existencia de un bracket factible, que es lo que \texttt{feasible\_beta\_bracket} verifica, sigue siendo una condición numérica en la calibración base).}
