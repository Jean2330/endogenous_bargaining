...
$\Pi_t$ is strictly concave ($\Pi_t''=-2\mu<0$). By (i) and (iii), $\Pi_t(0)>\overline\Pi>\Pi_t(1)$; by continuity (Intermediate Value Theorem) there is $\widehat\beta_\Pi\in(0,1)$ with $\Pi_t(\widehat\beta_\Pi)=\overline\Pi$, and by the fact above $\Pi_t'(\widehat\beta_\Pi)<0$; concavity gives $\Pi_t(\beta)>\overline\Pi \iff \beta<\widehat\beta_\Pi$.\\
...
\]
is continuous, with $\Pi_t(\beta)-\overline\Pi\to0^+$ and $\Pi_t'(\beta)\to\Pi_t'(\widehat\beta_\Pi)<0$ as $\beta\to\widehat\beta_\Pi^-$, so the second component stays finite while the first diverges to $-\infty$: $G(\beta)\to-\infty$. Symmetrically, $\text{CE}_t(\beta)-\overline{\text{CE}}\to0^+$ and $\text{CE}_t'(\beta)\to\text{CE}_t'(\widehat\beta_{\mathrm{CE}})>0$ as $\beta\to\widehat\beta_{\mathrm{CE}}^+$, so $G(\beta)\to+\infty$. By the Intermediate Value Theorem, $G$ has a root in $(\widehat\beta_{\mathrm{CE}},\widehat\beta_\Pi)\subset(0,1)$. To see that this root is $\beta_t^*$ itself, let $N(\beta):=[\Pi_t(\beta)-\overline\Pi]^{1-\delta}[\text{CE}_t(\beta)-\overline{\text{CE}}]^{\delta}$ denote the Nash product; $N$ extends continuously to $[\widehat\beta_{\mathrm{CE}},\widehat\beta_\Pi]$ with $N(\widehat\beta_{\mathrm{CE}})=N(\widehat\beta_\Pi)=0$ (one factor vanishes at each endpoint) and $N>0$ somewhere in between, by the nonemptiness of the feasible bracket. By the Extreme Value Theorem, $N$ attains its maximum over $[\widehat\beta_{\mathrm{CE}},\widehat\beta_\Pi]$ at an interior point, where $N$ is differentiable; that point is therefore a critical point of $N$, hence a root of $G=(\log N)'$. This identifies $\beta_t^*\in(\widehat\beta_{\mathrm{CE}},\widehat\beta_\Pi)\subset(0,1)$ directly, without invoking global concavity of the Nash product. (When $\mu\ge\kappa$, Proposition~\ref{prop:frontier_concavity} additionally gives unconditional concavity of the constrained Pareto frontier, the standard sufficient condition for uniqueness of the Nash bargaining solution; we do not verify that implication here, since it is not needed for the boundedness claim above.)\\
\textbf{Case $\mu<\kappa$.} Now $\text{CE}_t''(\beta)=\mu-\kappa<0$, so $\text{CE}_t$ is itself strictly concave, with $\text{CE}_t'(\beta)=(\mu-\kappa)\beta+|\underline y|$ strictly decreasing in $\beta$: positive for $\beta<\tilde\beta$, zero at $\tilde\beta:=|\underline y|/(\kappa-\mu)>0$, and negative for $\beta>\tilde\beta$. $\Pi_t$ remains strictly concave exactly as above, so (i) and (iii) give the same $\widehat\beta_\Pi\in(0,1)$ with $\Pi_t(\widehat\beta_\Pi)=\overline\Pi$, $\Pi_t'(\widehat\beta_\Pi)<0$, and $\Pi_t(\beta)>\overline\Pi \iff \beta<\widehat\beta_\Pi$.\\
...
When $\mu<\kappa$, the same argument applies verbatim wherever $\text{CE}_t'(\beta_t^*)>0$, which by Lemma \ref{lem:beta_bounds} (case $\mu<\kappa$) holds whenever $\beta_t^*<\tilde\beta=|\underline y|/(\kappa-\mu)$; the opposite conclusion, $\lambda'(W_t)>0$, holds on the complementary range $\beta_t^*>\tilde\beta$, since there $\text{CE}_t'(\beta_t^*)<0$ flips the sign of every step above.
}

\textcolor{olive}{PENDIENTE: figura nueva, dirección corregida respecto a la versión de la tesina (donde $\lambda$ subía desde abajo de $\delta$): aquí $\lambda_t$ empieza por ARRIBA de $\delta$ en $W$ bajo y desciende hasta cruzar $\delta$ en $\overline W$, porque el agente restringido en riqueza captura una fracción mayor que $\delta$. Ver Proposición \ref{prop:lambda_dynamics}.}
