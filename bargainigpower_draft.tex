...
$\Pi_t$ is strictly concave ($\Pi_t''=-2\mu<0$). By (i) and (iii), $\Pi_t(0)>\overline\Pi>\Pi_t(1)$; by continuity (Intermediate Value Theorem) there is $\mathrm{hi}\in(0,1)$ with $\Pi_t(\mathrm{hi})=\overline\Pi$, and by the fact above $\Pi_t'(\mathrm{hi})<0$; concavity gives $\Pi_t(\beta)>\overline\Pi \iff \beta<\mathrm{hi}$.\\
...
\]
is continuous, with $\Pi_t(\beta)-\overline\Pi\to0^+$ and $\Pi_t'(\beta)\to\Pi_t'(\mathrm{hi})<0$ as $\beta\to\mathrm{hi}^-$, so the second component stays finite while the first diverges to $-\infty$: $G(\beta)\to-\infty$. Symmetrically, $\text{CE}_t(\beta)-\overline{\text{CE}}\to0^+$ and $\text{CE}_t'(\beta)\to\text{CE}_t'(\mathrm{lo})>0$ as $\beta\to\mathrm{lo}^+$, so $G(\beta)\to+\infty$. By the Intermediate Value Theorem, $G$ has a root in $(\mathrm{lo},\mathrm{hi})\subset(0,1)$. To see that this root is $\beta_t^*$ itself, let $N(\beta):=[\Pi_t(\beta)-\overline\Pi]^{1-\delta}[\text{CE}_t(\beta)-\overline{\text{CE}}]^{\delta}$ denote the Nash product; $N$ extends continuously to $[\mathrm{lo},\mathrm{hi}]$ with $N(\mathrm{lo})=N(\mathrm{hi})=0$ (one factor vanishes at each endpoint) and $N>0$ somewhere in between, by the nonemptiness of the feasible bracket. By the Extreme Value Theorem, $N$ attains its maximum over $[\mathrm{lo},\mathrm{hi}]$ at an interior point, where $N$ is differentiable; that point is therefore a critical point of $N$, hence a root of $G=(\log N)'$. This identifies $\beta_t^*\in(\mathrm{lo},\mathrm{hi})\subset(0,1)$ directly, without invoking global concavity of the Nash product. (When $\mu\ge\kappa$, Proposition~\ref{prop:frontier_concavity} additionally gives unconditional concavity of the constrained Pareto frontier, the standard sufficient condition for uniqueness of the Nash bargaining solution; we do not verify that implication here, since it is not needed for the boundedness claim above.)\\
