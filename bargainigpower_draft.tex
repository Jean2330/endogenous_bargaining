...
\end{proposition}
At the baseline calibration ($\mu=0.5$, $\kappa=0.09$, $\underline{y}=-0.5$), $\mu > \kappa$, so the first, unconditional case applies: the frontier is concave everywhere in the binding regime, not merely at the reported equilibrium range.\\
}
...
\textcolor{blue}{
Throughout, fix $W_t$ in the binding regime and write $\mu:=\theta^2/k$, $\kappa:=\gamma\sigma^2$, so that $\Pi_t(\beta)=D(\beta)+\beta\underline y-\underline w_t$ and $\text{CE}_t(\beta)=C(\beta)-\beta\underline y+\underline w_t$ as in Case 2 of Section \ref{subsec:nash}. The proof below treats $\mu\ge\kappa$ (the baseline calibration) and $\mu<\kappa$ as two cases. \\
...
Numerically, at the baseline calibration and $W_t=0.5$: $\beta_t^*=0.7464$, $\Pi_t'(\beta_t^*)=-0.7464$, $\text{CE}_t'(\beta_t^*)=+0.8060$, $d\beta_t^*/d\underline w_t=-1.2146$, and $d\lambda_t/d\underline w_t=+0.2457$, so condition \eqref{eq:lambda_sign_condition} holds (verified against a finite-difference derivative of \texttt{solve\_beta\_static} and \texttt{lambda\_realized} in \texttt{model.py}, which matches the closed-form expression above to eight significant figures). By the chain rule this gives $d\beta_t^*/dW_t\approx+0.243$ and $d\lambda_t/dW_t\approx-0.049$, consistent with Section \ref{subsec:numericalresults}'s numerical result that $\lambda(W)$ falls from $\approx0.4446$ at low $W$ to $\delta=0.40$ at $\overline W$.\\
