...
\end{align*}
The terms in $\beta$ cancel exactly, so $M$ does not depend on $\beta$: it is a constant of the model's primitives. Since $\underline{y} < 0$ by construction (Section \ref{subsec:calibration}), $-\underline{y}(\mu+\kappa) > 0$, so $M > 0$ whenever $\mu \geq \kappa$, and more generally whenever $|\underline{y}|(\mu+\kappa) > \mu(\kappa-\mu)$.
...
\textbf{Nota relacionada:} el mismo hecho $\text{CE}_t'(\beta^*)>0$ (en vez de $<0$) que usamos aquí es el que, al propagarse al Apéndice B de la tesina (Proposición de dinámica de $\lambda(W)$), revierte el signo de $d\beta^*/d\underline{w}$ y por lo tanto de $\lambda'(W)$ respecto a lo que dice el Teorema 3.10 actual — confirmado numéricamente con \texttt{src/model.py}. Ver notas olive en \texttt{thesis\_draft.tex} (Teorema \ref{thm:central}, Apéndice \ref{ProofProp2}, Figura \ref{fig:wealth_dynamics}). Cuando lleguemos a portar la parte dinámica a este documento, hay que corregir ese signo también.
}
...
\textcolor{olive}{PENDIENTE (nota general sobre este apéndice): esta es la versión estática (para un $W_t$ dado) de la Lema C.1 de la tesina, no una copia. La tesina la prueba para funciones de continuación dinámicas $V_P^b, V_A^b$ y evalúa $F$ literalmente en $\beta=0,1$; aquí encontramos que esa evaluación literal no funciona, porque en $\beta=0,1$ ya falla la restricción de participación de una de las partes (verificado con \texttt{model.py}: $\text{CE}_s<0$ cerca de $\beta=0$, $\Pi_s<0$ cerca de $\beta=1$, en la calibración base). La prueba de abajo usa en cambio el bracket factible interior, que es lo que tu propio código (\texttt{feasible\_beta\_bracket}) ya hace en la práctica. Falta: (i) el caso $\mu<\kappa$ (aquí solo se hace $\mu\ge\kappa$, que es el caso de la calibración base); (ii) formalizar como Asunción las tres condiciones de regularidad usadas abajo, en vez de solo verificarlas numéricamente; (iii) el Apéndice B de la tesina (dinámica de $\lambda(W_t)$) sigue pendiente de traer aquí — tiene además el error de signo documentado en NOTES.md, no solo el problema de continuación vs.\ estático.}

\begin{appendices}
\section{Appendix: Boundedness of the equilibrium slope}\label{app:beta_bounds}

\textcolor{blue}{
Throughout, fix $W_t$ in the binding regime and write $\mu:=\theta^2/k$, $\kappa:=\gamma\sigma^2$, so that $\Pi_t(\beta)=D(\beta)+\beta\underline y-\underline w_t$ and $\text{CE}_t(\beta)=C(\beta)-\beta\underline y+\underline w_t$ as in Case 2 of Section \ref{subsec:nash}. We restrict attention to $\mu \ge \kappa$, the regime of the baseline calibration.
...
Suppose $\mu\ge\kappa$ and, at $W_t$,
\begin{enumerate}
